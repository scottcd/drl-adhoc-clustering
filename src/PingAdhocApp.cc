...
simsignal_t PingAdhocApp::rttSignal = registerSignal("rtt");
simsignal_t PingAdhocApp::numLostSignal = registerSignal("numLost");
simsignal_t PingAdhocApp::numOutOfOrderArrivalsSignal = registerSignal("numOutOfOrderArrivals");
simsignal_t PingAdhocApp::pingTxSeqSignal = registerSignal("pingTxSeq");
simsignal_t PingAdhocApp::pingRxSeqSignal = registerSignal("pingRxSeq");

const std::map<const Protocol *, const Protocol *> PingAdhocApp::l3Echo( {
...
void PingAdhocApp::parseDestAddressesPar()
{
...
void PingAdhocApp::handleMessageWhenUp(cMessage *msg)
{
...
void PingAdhocApp::socketClosed(INetworkSocket *socket)
{
...
void PingAdhocApp::refreshDisplay() const
{
...
void PingAdhocApp::handleStartOperation(LifecycleOperation *operation)
{
    if (isEnabled())
        startSendingPingRequests();
}

void PingAdhocApp::startSendingPingRequests()
{
...
void PingAdhocApp::handleStopOperation(LifecycleOperation *operation)
{
    pid = -1;
    lastStart = -1;
    sendSeqNo = expectedReplySeqNo = 0;
...
void PingAdhocApp::handleCrashOperation(LifecycleOperation *operation)
{
    pid = -1;
    lastStart = -1;
    sendSeqNo = expectedReplySeqNo = 0;
...
void PingAdhocApp::scheduleNextPingRequest(simtime_t previous, bool withSleep)
{
    simtime_t next;
...
void PingAdhocApp::cancelNextPingRequest()
{
    cancelEvent(timer);
}

bool PingAdhocApp::isEnabled()
{
    return par("destAddr").stringValue()[0] && (count == -1 || sentCount < count);
}

void PingAdhocApp::sendPingRequest()
{
    char name[32];
...
void PingAdhocApp::processPingResponse(int originatorId, int seqNo, Packet *packet)
{
...
void PingAdhocApp::countPingResponse(int bytes, long seqNo, simtime_t rtt, bool isDup)
{
...
std::vector<L3Address> PingAdhocApp::getAllAddresses()
{
